	...
	describe "#get_move_start" do
		...
		context "resolving positively" do 
			it do
				...
				expect(game.send(:get_move_start)).to be_an_instance_of Array
			end
		end
		context "resolving negatively" do 
			it do
				allow(game).to receive(:read_user_input){ [(9..100).to_a.sample, ("k".."z").to_a.sample].shuffle.join }
				expect(game.send(:get_move_start)).to be_nil
			end
		end
	end
	describe "#get_move_end" do
		let(:player1) do
			player1 = double("player 1")
			allow(player1).to receive(:colour){ "white" }
			player1
		...
		context "resolving positively" do 
			it do
				allow(game).to receive(:read_user_input){ [(1..8).to_a.sample, ("a".."h").to_a.sample].shuffle.join }
				expect(game.send(:get_move_end)).to be_an_instance_of Array
			end
		end
		context "resolving negatively" do 
			it do
				allow(game).to receive(:read_user_input){ [(9..100).to_a.sample, ("k".."z").to_a.sample].shuffle.join }
				expect(game.send(:get_move_end)).to be_nil
			end
		end
	end
end