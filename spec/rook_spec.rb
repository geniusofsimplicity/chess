	...
	describe "#valid_move?"	do
		...
		context "performing validly" do
			...
			it "a rank move up" do				
				expect(rook.valid_move?(move_from, move_in_rank_up_to, board)).to be_truthy
			end
			it "a rank move down" do				
				expect(rook.valid_move?(move_from, move_in_rank_down_to, board)).to be_truthy
			end
			it "a file move to the left" do				
				expect(rook.valid_move?(move_from, move_in_file_left_to, board)).to be_truthy
			end
			it "a file move to the right" do				
				expect(rook.valid_move?(move_from, move_in_file_right_to, board)).to be_truthy
			end
		end
		...
		context "performing invalidly" do
			...
			it "a leaping rank move up" do				
				expect(rook.valid_move?(move_from, move_in_rank_up_to, board_leap_rank_up)).to be_falsey
			end
			it "a leaping rank move down" do				
				expect(rook.valid_move?(move_from, move_in_rank_down_to, board_leap_rank_down)).to be_falsey
			end
			it "a file move to the left" do				
				expect(rook.valid_move?(move_from, move_in_file_left_to, board_leap_file_left)).to be_falsey
			end
			it "a file move to the right" do				
				expect(rook.valid_move?(move_from, move_in_file_right_to, board_leap_file_right)).to be_falsey
			end
		end
	end	
end